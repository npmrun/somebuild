...
async function buildDocs() {
    ...
    let pagesPromise = []
        for (let i = 0; i < renderData.navbar.length; i++) {
            const element = renderData.navbar[i];
            let htmlP = '/zh/' + element.path + '/index.html'
            if (htmlP === "/zh/docs/home/index.html") {
                htmlP = "index.html"
                pagesPromise.push(renderPage('/zh/' + element.path, htmlP, { manifest, clientResult, render, renderData, matchedStr }))
            } else {
                pagesPromise.push(renderPage('/zh/' + element.path, htmlP, { manifest, clientResult, render, renderData, matchedStr }))
                pagesPromise.push(renderPage('/en/' + element.path, htmlP.replace("zh", "en"), { manifest, clientResult, render, renderData, matchedStr }))
            }
        }
        for (let i = 0; i < renderData.action.length; i++) {
            const element = renderData.action[i];
            let htmlP = '/zh/' + element.path + '/index.html'
            if (htmlP === "/zh/docs/home/index.html") {
                htmlP = "index.html"
                pagesPromise.push(renderPage('/zh/' + element.path, htmlP, { manifest, clientResult, render, renderData, matchedStr }))
        ...
        for (let i = 0; i < renderData.sidebar.length; i++) {
            const element = renderData.sidebar[i];
            if (element.items) {
                for (let j = 0; j < element.items.length; j++) {
                    const item = element.items[j];
                    let htmlP = '/zh/' + item.path + '/index.html'
                    if (htmlP === "/zh/docs/home/index.html") {
                        htmlP = "index.html"
                        pagesPromise.push(renderPage('/zh/' + item.path, htmlP, { manifest, clientResult, render, renderData, matchedStr }))
                    } else {
                        pagesPromise.push(renderPage('/zh/' + item.path, htmlP, { manifest, clientResult, render, renderData, matchedStr }))
                        pagesPromise.push(renderPage('/en/' + item.path, htmlP.replace("zh", "en"), { manifest, clientResult, render, renderData, matchedStr }))
                    }
                }
            }
        }
